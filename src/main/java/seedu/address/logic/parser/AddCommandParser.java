...
public class AddCommandParser implements Parser<AddCommand> {
    ...
    public AddCommand parse(String args) throws ParseException {
        ArgumentMultimap argMultimap = ArgumentTokenizer.tokenize(args, PREFIX_NAME, PREFIX_PHONE, PREFIX_EMAIL,
                PREFIX_ADDRESS, PREFIX_SCHOOL, PREFIX_DEGREE, PREFIX_SKILL);

        if (!arePrefixesPresent(argMultimap, PREFIX_NAME, PREFIX_PHONE, PREFIX_EMAIL, PREFIX_ADDRESS, PREFIX_SCHOOL,
                PREFIX_DEGREE) || !argMultimap.getPreamble().isEmpty()) {
            throw new ParseException(String.format(MESSAGE_INVALID_COMMAND_FORMAT, AddCommand.MESSAGE_USAGE));
        }

        argMultimap.verifyNoDuplicatePrefixesFor(PREFIX_NAME, PREFIX_PHONE, PREFIX_EMAIL, PREFIX_ADDRESS, PREFIX_SCHOOL,
                PREFIX_DEGREE);
        Name name = ParserUtil.parseName(argMultimap.getValue(PREFIX_NAME).get());
        School school = ParserUtil.parseRemark(argMultimap.getValue(PREFIX_SCHOOL).get());
        ...
        Person person = new Person(name, phone, email, address, school, degree, skillList);
